...
void displayTime(int time, int* hours, int* mins, int* sec) {
	gotoxy(28,2);
	printf("%d Second(s) = %d Hours %d Minutes %d Second(s)",time, *hours, *mins, *sec);
	return;
}
...
void computeCoin(int coinValue, int* quarter, int* dime, int* penny) {
	*quarter = 0, *dime = 0, *penny = 0;
	...
	} else gotoxy(28,4);printf("Converts cents between 0-99!");
	return;
}

void displayCoin(int coinValue, int* quarter, int* dime, int* penny) {
	gotoxy(28,2);
	printf("%d Cents = %d quarter(s) %d dime(s) %d penny",coinValue, *quarter,*dime,*penny);
	return;
}
