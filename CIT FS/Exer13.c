...
int main() {
	...
	do {
			system("cls");menu();
			gotoxy(2, 16);printf ("Please enter a number [1 - 9]: ");
			scanf("%d", &n);
		switch(n) {
			case 1:
				gotoxy(4, 18);
			...
			case 3:
				...
				break;
			case 4:
				gotoxy(2, 18);printf("Please enter a row of your choice: ");
				scanf("%d", &choice);
				gotoxy(2, 20);
				printf ("The sum of the row is %d", rowSum(arr, choice));
				break;
			case 5:
				gotoxy(2, 18);
				printf("Please enter a column of your choice: ");
				scanf("%d", &choice);
				gotoxy(2, 20);
				printf("The sum of the column is %d", columnSum(arr, choice));
				break;
			case 6:
...
//				gotoxy(2, 18);printf(isColumnMagic(arr) ? "It is a Magic column!" : "It is not a Magic column");  // This is an option to shorten code by TERNARY operation and another style of operation for conditional statements
				if (isColumnMagic(arr)) {
					gotoxy(2, 18);
					printf ("It is a Magic column!");
				} else  {
					gotoxy(2, 18);
					printf ("It is not a Magic column");
				}
				break;
			case 8:
//				gotoxy(2, 18);printf(isMagic(arr) ? "It is a Magic square!" : "It is not a Magic square!");  // This is an option to shorten code by TERNARY operation and another style of operation for conditional statements
			...
			case 9:
				...
				Exit();
				break;
			default: gotoxy(2, 18);printf ("Please enter a valid number!");
		}
		gotoxy(37,0);
		system("pause");
	} while (rep == 'y' || 'Y');
...
int isRowMagic(int arr[][COL]) {
	...
	if(temp == rowSum(arr, 1) && temp == rowSum(arr, 2)) return 1;
	else return 0;
}

int isColumnMagic(int arr[][COL]) {
	int temp = columnSum(arr, 0);
	if (temp == columnSum(arr, 1) && temp == columnSum(arr, 2)) return 1;
	else return 0;
}

int isMagic(int arr[][COL]) {
	if (isRowMagic(arr) && isColumnMagic) return 1;
	else return 0;
}
